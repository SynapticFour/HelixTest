...
fn resolve_and_get_schema(yaml: &str, schema_name: &str, context: &str) -> Result<Value> {
    let openapi_value: Value = serde_yaml::from_str(yaml)
        .with_context(|| format!("{}: parse error", context))?;
    let doc = openapi_deref::resolve(&openapi_value).map_err(|e| {
        anyhow::anyhow!("Failed to resolve {}: {:?}", context, e)
    })?;
...
fn load_wes_service_info_schema() -> Result<JSONSchema> {
    let service_info = resolve_and_get_schema(
        WES_OPENAPI_YAML,
        "ServiceInfo",
        "WES OpenAPI",
    )?;
...
fn validate_against(schema: &JSONSchema, value: &Value, schema_label: &str) -> Result<()> {
    schema.validate(value).map_err(|errors| {
        ...
        anyhow::anyhow!("JSON did not validate against {}: {}", schema_label, msgs.join("; "))
    })?;
    Ok(())
}
...
pub fn validate_tes_create_task_response(value: &Value) -> Result<()> {
    let schema = TES_CREATE_TASK_RESPONSE_SCHEMA.get_or_try_init(load_tes_create_task_response_schema)?;
...
mod tests {
    ...
    fn tes_create_task_response_requires_id() {
        ...
        validate_tes_create_task_response(&minimal).expect("tesCreateTaskResponse with id should validate");
    }
