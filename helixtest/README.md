...
Create `helixtest-config.toml` in the `helixtest` directory:
...
```bash
export HELIXTEST_CONFIG=/path/to/helixtest-config.toml
```
...
1. **Point HelixTest at your services** – Set the base URLs via a config file (`helixtest-config.toml` or `HELIXTEST_CONFIG`) or environment variables (`WES_URL`, `TES_URL`, etc.).
2. **Run the suite** – `cargo run --bin helixtest -- --all` (optionally `--report json` or `--report scores` for machine-readable output).
...
```bash
cargo run --bin helixtest -- --all --report table
...
```bash
cargo run --bin helixtest -- --all --report json --fail-level 2 > report.json
echo $?  # 0 = pass, 1 = fail
```
...
- Improved schema coverage using [official GA4GH OpenAPI/JSON schemas](docs/ga4gh-official-schemas.md) (see that doc for an implementation outline).
...
- `HELIXTEST_CONFIG=./helixtest-config.toml` (optional config file path)
