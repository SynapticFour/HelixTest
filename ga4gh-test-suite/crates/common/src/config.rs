...
impl TestConfig {
    pub fn from_env_or_file() -> Result<Self> {
        // Highest precedence: GA4GH_TEST_PROFILE pointing at profiles/<name>.toml
        if let Ok(profile) = env::var("GA4GH_TEST_PROFILE") {
            ...
            let data = fs::read_to_string(&path).with_context(|| {
                format!(
                    "Failed to read profile config at {} (from GA4GH_TEST_PROFILE)",
                    path.display()
                )
            })?;
            let cfg: TestConfig =
        ...
        if let Ok(path) = env::var("GA4GH_TEST_CONFIG") {
            let p = Path::new(&path);
        ...
        let default_path = Path::new("ga4gh-test-config.toml");
