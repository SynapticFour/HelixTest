...
fn auth_secret() -> String {
    std::env::var("GA4GH_TEST_SHARED_SECRET").unwrap_or_else(|_| "test-secret".to_owned())
}

/// Test object ID used for auth checks (DRS GET with Bearer). Override via GA4GH_TEST_AUTH_OBJECT_ID.
fn auth_test_object_id() -> String {
    std::env::var("GA4GH_TEST_AUTH_OBJECT_ID").unwrap_or_else(|_| "test-object-1".to_owned())
}
