...
async fn level1_task_schema(cfg: &TestConfig, client: &HttpClient) -> TestCaseResult {
    let res = async {
        ...
        // Basic schema validation on createTask response
        validate_json::<TesTask>(&v)?;
        ...
        // Single status fetch for schema (no lifecycle enforcement here)
        ...
        validate_json::<TesTask>(&status_val)?;
...
fn validate_json<T: JsonSchema + for<'de> Deserialize<'de> + Serialize>(v: &Value) -> Result<()> {
    let schema = schemars::schema_for!(T);
    let schema_val = serde_json::to_value(&schema.schema)?;
    let schema_static: &'static serde_json::Value = Box::leak(Box::new(schema_val));
    let compiled = jsonschema::JSONSchema::compile(schema_static)?;
    if let Err(iter) = compiled.validate(v) {
