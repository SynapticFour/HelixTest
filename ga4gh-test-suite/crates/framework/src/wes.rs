...
async fn level1_service_info_schema(cfg: &TestConfig, client: &HttpClient) -> TestCaseResult {
    ...
    match res {
        Ok(v) => {
            let schema = schemars::schema_for!(WesServiceInfo);
            let schema_val = serde_json::to_value(&schema.schema).unwrap();
            let compiled = jsonschema::JSONSchema::compile(&schema_val).unwrap();
            let mut errors = Vec::new();
            if let Err(iter) = compiled.validate(&v) {
                for e in iter {
                    errors.push(format!("{} at {}", e, e.instance_path));
                }
            }
            let ok_version = v
            ...
            TestCaseResult {
                name: "WES service-info schema & enums".into(),
