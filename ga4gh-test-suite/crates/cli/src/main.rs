...
#[derive(Parser, Debug)]
#[command(name = "ga4gh-test-suite")]
#[command(version = env!("CARGO_PKG_VERSION"))]
#[command(about = "HelixTest — GA4GH Conformance Suite")]
#[command(after_help = "Synaptic Four — Built with ❤️ for the open science community. GA4GH open standards for sovereign bioinformatics. Proudly developed by individuals on the autism spectrum in Germany. © 2025 Synaptic Four · Apache-2.0.")]
struct Args {
...
async fn main() -> Result<()> {
    ...
    if args.verbose && std::env::var("RUST_LOG").is_err() {
        std::env::set_var("RUST_LOG", "debug,ga4gh_test_suite=debug");
    }
    init_logging();
    if args.all {
        println!("{}", BANNER);
