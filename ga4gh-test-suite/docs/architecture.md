...
┌─────────────────────────────────────────────────────────────────────────┐
│  User / CI                                                               │
│  ga4gh-test-suite --all [--report table|json|scores|coverage]           │
└─────────────────────────────────────────────────────────────────────────┘
                                      │
                                      ▼
┌─────────────────────────────────────────────────────────────────────────┐
...
│  • Loads config (GA4GH_TEST_PROFILE / GA4GH_TEST_CONFIG / env)          │
...
1. **Config** – Endpoints come from `GA4GH_TEST_PROFILE` → `profiles/<name>.toml`, or `GA4GH_TEST_CONFIG`, or `ga4gh-test-config.toml`, or `WES_URL`/… environment variables.
...
- **Framework** (`crates/framework`) – Conformance runs used by the CLI: one entrypoint `run_all()`, produces `OverallReport`. This is what `ga4gh-test-suite --all` runs.
