...
1. **Use the Ferrum profile** (endpoints + feature flags):
   ```bash
   export GA4GH_TEST_PROFILE=ferrum
   ```
...
2. **Optional: start Ferrum with Docker** (from the suite or project root):
   ...
   ```bash
   ga4gh-test-suite --all --mode ferrum --start-ferrum
   ```

3. **Run the suite**:
   ```bash
   cargo run --bin ga4gh-test-suite -- --all --mode ferrum
   ```
...
Override endpoints via `GA4GH_TEST_CONFIG` or environment variables (`WES_URL`, etc.) if your deployment uses different ports or hosts.

## Mode: generic vs ferrum

- **`--mode ferrum`** – Loads features from `profiles/ferrum.toml` (if no `GA4GH_TEST_PROFILE` is set) and skips Ferrum auto-detection.
...
Using `GA4GH_TEST_PROFILE=ferrum` is equivalent for feature/endpoint loading and works with either mode.
