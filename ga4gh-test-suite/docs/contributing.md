...
### 1. Conformance tests (run by `ga4gh-test-suite --all`)
...
4. **Rebuild and run** – `cargo run --bin ga4gh-test-suite -- --all` will include your test.
...
- Run `cargo build -p ga4gh-test-suite-cli` and `cargo test` (for the crates you touch) before submitting.
...
- [ ] **Legal:** LICENSE and NOTICE are in repo root; README and docs reference [DISCLAIMER](DISCLAIMER.md) and state that test results are not official certification.
- [ ] **Wording:** No unconditional “guarantee” or “certified”; use “conformance checks,” “for informational use,” “as is” where appropriate.
- [ ] **CI:** `cargo build --bin ga4gh-test-suite` and `cargo test --workspace` pass.
- [ ] **Config:** Default config or env fallbacks are documented; no secrets or internal URLs in repo.
- [ ] **Attribution:** Synaptic Four USP and copyright (© 2025) appear in README, NOTICE, and CLI (banner/help).
